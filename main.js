...
function loadSessionMap() {
    try {
        ...
        if (fs.existsSync(sessionMapPath)) {
            const data = fs.readFileSync(sessionMapPath, 'utf-8');
...
ipcMain.handle('get-current-ip', () => {
    return { currentIP: currentUserPublicIP };
});
...
ipcMain.handle('get-or-create-account', async (_, { name, number }) => {
    ...
    if (accountInfo) {
        ...
        const partitionId = `persist:whatsapp-session-${newWindowId}`;
    ...
    await senderWin.webContents.executeJavaScript(`
      new Promise((resolve, reject) => {
        ...
        const interval = setInterval(() => {
          attempt++;
          const commonTexts = ['continue', 'ok', 'got it'];
          const actionButton = Array.from(document.querySelectorAll('button')).find(b => commonTexts.includes(b.textContent.trim().toLowerCase()));
          if (actionButton) actionButton.click();
          const sendBtn = document.querySelector('button[aria-label="Send"], span[data-icon="send"], button[data-tab="11"]');
          if (sendBtn && document.querySelector('#main')) {
            clearInterval(interval);
            resolve(true);
          }
          if (attempt > 60) {
            clearInterval(interval);
            reject(new Error('Send button or main panel not found after 30 seconds.'));
          }
        }, 500);
      });
    `);
    const result = await senderWin.webContents.executeJavaScript(`
      (() => {
        const sendBtn = document.querySelector('button[aria-label="Send"], span[data-icon="send"], button[data-tab="11"]');
        if (sendBtn) {
          sendBtn.click();
...
ipcMain.handle('send-group-message', async (_, { senderWindowId, groupName, message }) => {
    ...
    try {
        ...
        const groupElementBoundsJSON = await senderWin.webContents.executeJavaScript(`
            new Promise(async (resolve, reject) => {
                const delay = ms => new Promise(res => setTimeout(res, ms));
                const searchBox = document.querySelector('div[contenteditable="true"][data-tab="3"]');
                if (!searchBox) return reject(new Error('Search box not found'));
                searchBox.focus();
                ...
                const findInterval = setInterval(() => {
                    findAttempt++;
                    const listItems = Array.from(document.querySelectorAll('#pane-side div[role="listitem"]'));
                    let foundItem = null;
                    for (const item of listItems) {
                        const titleSpan = item.querySelector('span[title]');
                        if (titleSpan && titleSpan.getAttribute('title').trim().toLowerCase() === '${groupName.replace(/'/g, "\\'").trim().toLowerCase()}') {
                            foundItem = item;
                            break;
                        }
                    }
                    if (foundItem) {
                        ...
                        const bounds = foundItem.getBoundingClientRect();
                        resolve(JSON.stringify(bounds));
                    } else if (findAttempt > 20) {
                        clearInterval(findInterval);
                        reject(new Error('Group "${groupName}" found, but failed to get its coordinates.'));
                    }
                }, 500);
            });
        `);
        ...
        const result = await senderWin.webContents.executeJavaScript(`
             new Promise((resolve, reject) => {
                let attempt = 0;
                const messageBoxInterval = setInterval(() => {
                    attempt++;
                    ...
                    if (messageBox) {
                        ...
                        document.execCommand('paste');
                        setTimeout(() => {
                            const sendBtn = document.querySelector('button[aria-label="Send"]') || document.querySelector('span[data-icon="send"]');
                            if (!sendBtn) return reject(new Error('Send button not found.'));
                            sendBtn.click();
                            resolve('sent');
                        }, 500);
                    } else if (attempt > 40) {
...
ipcMain.handle('find-and-reply-story', async (_, accountId) => {
    ...
    try {
        ...
        await win.webContents.executeJavaScript(`
            new Promise((resolve, reject) => {
                let attempt = 0;
                const findStatusTabInterval = setInterval(() => {
                    attempt++;
                    const statusTab = document.querySelector('button[aria-label*="Status"]');
                    if (statusTab) {
                        clearInterval(findStatusTabInterval);
                        statusTab.click();
                        resolve(true);
                    }
                    if (attempt > 60) {
                        clearInterval(findStatusTabInterval);
                        reject(new Error('Tombol Status tidak ditemukan setelah 30 detik.'));
                    }
                }, 500);
            });
        `);
        await new Promise(resolve => setTimeout(resolve, 2000));
        const storyElementBoundsJSON = await win.webContents.executeJavaScript(`
            new Promise((resolve, reject) => {
                let storyToClick = null;
                const statusItems = Array.from(document.querySelectorAll('div[class*="statusList"] div[role="listitem"]'));
                for (const item of statusItems) {
                    const unreadIndicator = item.querySelector('svg[class*="x165d6jo"]');
                    if (unreadIndicator) {
                        storyToClick = item.querySelector('div[role="button"]');
                        break;
                    }
                }
                if (storyToClick) {
                    const bounds = storyToClick.getBoundingClientRect();
                    resolve(JSON.stringify({ bounds: bounds, owner: storyToClick.querySelector('span[title]')?.getAttribute('title') || 'a friend' }));
                } else {
                    resolve(JSON.stringify({ bounds: null, owner: null }));
                }
            });
        `);
        ...
        const replyResult = await win.webContents.executeJavaScript(`
            new Promise(async (resolve, reject) => {
                let attempt = 0;
                const findReplyUIInterval = setInterval(() => {
                    attempt++;
                    const replyBox = document.querySelector('div[role="textbox"][aria-placeholder="Type a reply…"]');
                    const sendButtonContainer = document.querySelector('button span[data-icon="send"]');
                    const sendButton = sendButtonContainer ? sendButtonContainer.closest('button') : null;
                    if (replyBox && sendButton) {
                        clearInterval(findReplyUIInterval);
                        replyBox.focus();
                        document.execCommand('paste');
                        setTimeout(() => {
                            sendButton.click();
                    ...
                    if (attempt > 40) {
                        clearInterval(findReplyUIInterval);
                        reject(new Error('Reply UI (box or send button) not found in story viewer.'));
                    }
                }, 500);
            });
        `);
        ...
        await win.webContents.executeJavaScript(`
            ...
            setTimeout(() => {
                const chatTab = document.querySelector('button[aria-label*="Chat"]');
    ...
    try {
        ...
        const result = await win.webContents.executeJavaScript(`
            new Promise(async (resolve, reject) => {
                ...
                try {
                    const statusTab = document.querySelector('button[aria-label*="Status"]');
                    if (!statusTab) throw new Error('Status tab not found');
                    statusTab.click();
                    await delay(1500);
                    const addStatusBtn = document.querySelector('button[aria-label="Add Status"]');
                    if (!addStatusBtn) throw new Error('Add Status (+) button not found');
                    addStatusBtn.click();
                    await delay(1500);
                    const pencilIcon = document.querySelector('span[data-icon="pencil-refreshed"]');
                    const createTextStatusBtn = pencilIcon ? pencilIcon.closest('li[role="button"]') : null;
                    if (!createTextStatusBtn) throw new Error('Create text status button (pencil icon) not found');
                    createTextStatusBtn.click();
                    await delay(2000);
                    const textEditor = document.querySelector('div[contenteditable="true"][aria-placeholder="Type a status"]');
                    if (!textEditor) throw new Error('Status text editor not found');
                    textEditor.focus();
                    document.execCommand('paste');
                    await delay(500);
                    const sendButton = document.querySelector('div[role="button"][aria-label="Send"]');
                    if (!sendButton) throw new Error('Send status button not found');
                    sendButton.click();
                    await delay(2000);
                    await delay(1000); 
                    const chatTab = document.querySelector('button[aria-label*="Chat"]');
